...
gr_papyrus_ofdm_insert_preamble::gr_papyrus_ofdm_insert_preamble
  ...
  : gr_block("ofdm_insert_preamble",
    ...
    d_pending_flag(0)
{
  // linklab, allocate non-contiguous preamble
  d_preamble_nc.assign(d_preamble.begin(), d_preamble.end());

  // sanity check preamble symbols
  for(size_t i = 0; i < d_preamble.size(); i++) {
    if(d_preamble[i].size() != (size_t) d_fft_length)
      throw std::invalid_argument("gr_papyrus_ofdm_insert_preamble: invalid length for preamble symbol");
  }
...
int
gr_papyrus_ofdm_insert_preamble::general_work (int noutput_items,
...
#define write_out_flag() 			\
  ...
  while (no < noutput_items && ni < ninput_items){
    ...
    case ST_PREAMBLE:
      if (VERBOSE_1)
         printf("%s: d_state ST_PREAMBLE preamble_nc_size %d\n", __FUNCTION__, (int) d_preamble_nc.size());
      assert(in_flag[ni] & 0x1);
      if (d_nsymbols_output >= (int) d_preamble_nc.size()){ // linklab, non-contiguous preamble
	// we've output all the preamble
	enter_first_payload();
      }
      else {
	memcpy(&out_sym[no * d_fft_length],
	       &d_preamble_nc[d_nsymbols_output][0], // linklab, non-contiguous preamble
    ...
    case ST_FIRST_PAYLOAD:
      if (VERBOSE_1)
        printf("d_state ST_FIRST_PAYLOAD\n");
    ...
    case ST_PAYLOAD:
      if (in_flag && in_flag[ni] & 0x1){	// this is first symbol of a new payload
        if (VERBOSE_1)
           printf("%s: ST_PAYLOAD entering preamble\n",__FUNCTION__);
	enter_preamble();
	break;
      }
    ...
    default:
      std::cerr << "gr_papyrus_ofdm_insert_preamble: (can't happen) invalid state, resetting\n";
      enter_idle();
    }
  }
...
void
gr_papyrus_ofdm_insert_preamble::enter_idle()
{
  d_state = ST_IDLE;
  d_nsymbols_output = 0;
  d_pending_flag = 0;
...
void
gr_papyrus_ofdm_insert_preamble::enter_preamble()
{
  d_state = ST_PREAMBLE;
  d_nsymbols_output = 0;
  d_pending_flag = 1;
...
void
gr_papyrus_ofdm_insert_preamble::enter_first_payload()
{
  d_state = ST_FIRST_PAYLOAD;
}

void
gr_papyrus_ofdm_insert_preamble::enter_payload()
{
  d_state = ST_PAYLOAD;
}

// reset preamble, linklab
void
gr_papyrus_ofdm_insert_preamble::reset_preamble(std::vector<std::vector<gr_complex> > new_preamble)
{
  d_preamble_nc.clear();  // linklab, reset non-contiguous preamble
  d_preamble_nc.assign(new_preamble.begin(), new_preamble.end());  // assign new preamble
}
